...
# ---------------------------------------------------------------------------
# 13. realized_pnl — monetary P&L on the exited portion (issue #26)
# ---------------------------------------------------------------------------


def test_realized_pnl_closed_long_profit() -> None:
  with TestClient(app) as client:
    asset_id = _create_asset(client)
    # Long: entry 100 qty 2, exit 110 qty 2 → reward +10, exit_qty 2 → +20.
    response = client.post(
      "/api/trades",
      json={
        "asset_id": asset_id,
        "activities": [
          {"type": "Buy", "price": 100.0, "quantity": 2.0, "date": "2024-11-01"},
          {"type": "Sell", "price": 110.0, "quantity": 2.0, "date": "2024-11-02"},
        ],
      },
    )

  assert response.status_code == 201
  assert response.json()["data"]["realized_pnl"] == pytest.approx(20.0)


def test_realized_pnl_closed_long_loss() -> None:
  with TestClient(app) as client:
    asset_id = _create_asset(client)
    # Long: entry 100 qty 2, exit 90 qty 2 → reward -10, exit_qty 2 → -20.
    response = client.post(
      "/api/trades",
      json={
        "asset_id": asset_id,
        "activities": [
          {"type": "Buy", "price": 100.0, "quantity": 2.0, "date": "2024-11-01"},
          {"type": "Sell", "price": 90.0, "quantity": 2.0, "date": "2024-11-02"},
        ],
      },
    )
...
def test_realized_pnl_closed_short_profit() -> None:
  with TestClient(app) as client:
    asset_id = _create_asset(client)
    # Short: entry 100 (Sell) qty 2, exit 90 (Buy) qty 2 → reward +10 → +20.
    response = client.post(
      "/api/trades",
      json={
        "asset_id": asset_id,
        "activities": [
          {"type": "Sell", "price": 100.0, "quantity": 2.0, "date": "2024-11-01"},
          {"type": "Buy", "price": 90.0, "quantity": 2.0, "date": "2024-11-02"},
        ],
      },
    )
...
def test_realized_pnl_closed_short_loss() -> None:
  with TestClient(app) as client:
    asset_id = _create_asset(client)
    # Short: entry 100 (Sell) qty 2, exit 110 (Buy) qty 2 → reward -10 → -20.
    response = client.post(
      "/api/trades",
      json={
        "asset_id": asset_id,
        "activities": [
          {"type": "Sell", "price": 100.0, "quantity": 2.0, "date": "2024-11-01"},
          {"type": "Buy", "price": 110.0, "quantity": 2.0, "date": "2024-11-02"},
        ],
      },
    )
...
def test_realized_pnl_partial_close_counts_exited_qty_only() -> None:
  with TestClient(app) as client:
    asset_id = _create_asset(client)
    # Long: entry 100 qty 10, exit 110 qty 4 → reward +10, only 4 exited → +40.
    response = client.post(
      "/api/trades",
      json={
        "asset_id": asset_id,
        "activities": [
          {"type": "Buy", "price": 100.0, "quantity": 10.0, "date": "2024-11-01"},
          {"type": "Sell", "price": 110.0, "quantity": 4.0, "date": "2024-11-02"},
        ],
      },
    )
  ...
  assert data["realized_pnl"] == pytest.approx(40.0)


def test_realized_pnl_open_trade_is_null() -> None:
  with TestClient(app) as client:
    asset_id = _create_asset(client)
    response = _create(
      client,
      asset_id,
      activities=[{"type": "Buy", "price": 100.0, "quantity": 1.0, "date": "2024-11-01"}],
    )
...
def test_realized_pnl_missed_opportunity_is_null() -> None:
  with TestClient(app) as client:
    asset_id = _create_asset(client)
    # A fully closed trade, but flagged as a missed opportunity → null.
    response = client.post(
      "/api/trades",
      json={
        "asset_id": asset_id,
  ...
  assert response.status_code == 201
  data = response.json()["data"]
  assert data["missed_opportunity"] is True
  assert data["realized_pnl"] is None


def test_realized_pnl_breakeven_is_zero() -> None:
  with TestClient(app) as client:
    asset_id = _create_asset(client)
    # Exit price == entry price → 0.0 (not null: it is realized, just flat).
    response = client.post(
      "/api/trades",
      json={
        "asset_id": asset_id,
        "activities": [
          {"type": "Buy", "price": 100.0, "quantity": 2.0, "date": "2024-11-01"},
          {"type": "Sell", "price": 100.0, "quantity": 2.0, "date": "2024-11-02"},
        ],
      },
    )
...
def test_realized_pnl_rounded_to_five_decimals() -> None:
  with TestClient(app) as client:
    asset_id = _create_asset(client)
    # reward = 1.234567 - 1.123456 = 0.111111; × 3 = 0.333333 → 0.33333.
    response = client.post(
      "/api/trades",
      json={
        "asset_id": asset_id,
        "activities": [
          {"type": "Buy", "price": 1.123456, "quantity": 3.0, "date": "2024-11-01"},
          {"type": "Sell", "price": 1.234567, "quantity": 3.0, "date": "2024-11-02"},
        ],
      },
    )

  assert response.status_code == 201
  assert response.json()["data"]["realized_pnl"] == pytest.approx(0.33333)


def test_realized_pnl_in_get_detail() -> None:
  with TestClient(app) as client:
    asset_id = _create_asset(client)
    trade_id = client.post(
      "/api/trades",
  ...
  assert detail.status_code == 200
  assert detail.json()["data"]["realized_pnl"] == pytest.approx(20.0)


def test_realized_pnl_in_list_summaries() -> None:
  with TestClient(app) as client:
    asset_id = _create_asset(client)
    # One closed trade (P&L set) and one open trade (P&L null).
    client.post(
      "/api/trades",
      json={
        "asset_id": asset_id,
        "activities": [
          {"type": "Buy", "price": 100.0, "quantity": 2.0, "date": "2024-11-02"},
          {"type": "Sell", "price": 110.0, "quantity": 2.0, "date": "2024-11-03"},
        ],
      },
    )
    _create(client, asset_id, activities=[{"type": "Buy", "price": 100.0, "quantity": 1.0, "date": "2024-11-01"}])
    response = client.get("/api/trades")

  assert response.status_code == 200
  data = response.json()["data"]
  by_date = {trade["trade_date"]: trade for trade in data}
  assert by_date["2024-11-02"]["realized_pnl"] == pytest.approx(20.0)
  assert by_date["2024-11-01"]["realized_pnl"] is None


def test_update_recomputes_realized_pnl() -> None:
  with TestClient(app) as client:
    asset_id = _create_asset(client)
    # Start open (null), then add an exit via activity replacement.
    trade_id = _create(
      client,
      asset_id,
      activities=[{"type": "Buy", "price": 100.0, "quantity": 2.0, "date": "2024-11-01"}],
    ).json()["data"]["id"]
    assert client.get(f"/api/trades/{trade_id}").json()["data"]["realized_pnl"] is None
    ...
    response = client.put(
      ...
      json={
        "activities": [
          {"type": "Buy", "price": 100.0, "quantity": 2.0, "date": "2024-11-01"},
          {"type": "Sell", "price": 105.0, "quantity": 2.0, "date": "2024-11-02"},
        ]
      },
    )

  assert response.status_code == 200
  assert response.json()["data"]["realized_pnl"] == pytest.approx(10.0)


def test_update_missed_opportunity_nulls_realized_pnl() -> None:
  with TestClient(app) as client:
    asset_id = _create_asset(client)
    # Closed trade with a P&L, then flag it as a missed opportunity → null.
    trade_id = client.post(
      "/api/trades",
      json={
        "asset_id": asset_id,
    ...
    assert client.get(f"/api/trades/{trade_id}").json()["data"]["realized_pnl"] == pytest.approx(20.0)

    response = client.put(f"/api/trades/{trade_id}", json={"missed_opportunity": True})
