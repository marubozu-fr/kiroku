...
class FuturesService:
  """Resolves a futures base symbol to its active contract ticker."""

  async def resolve_contract(self, base_symbol: str, trade_date: date) -> str:
    ...
    contract = self._select_active_contract(contracts, trade_date)
    if contract is None:
      raise FuturesResolutionError(
  ...
  def _select_active_contract(
    contracts: list[dict], trade_date: date
  ) -> dict | None:
